...
namespace lcxx {
     ...
     * \details
     * The caller, when it doesn't find a valid license (see `acquire_license` below) should show to the user the
     * calculated identifier. The user should report the pc_identifier to the software editor that will in turn use it
     * to issue a license.
     *
     * pc_id_method = ::STRATEGY_DEFAULT is usually the best choice.
     *
     * First call this method with `identifier_out` = nullptr and `buf_size` = 0 , it will return the requested buffer
     * size in `buf_size` parameter.
     *
     ...
     * @param hw_id_method[in] specifies a preferred identification method. Usually #STRATEGY_DEFAULT works well. See
     ...
     * 			contains informations on the software that is requesting the license verification. Let the
     * software specify its version or request verification for features that need to be enabled separately.
