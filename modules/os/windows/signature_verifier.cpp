...
#define NT_SUCCESS( Status ) ( ( (NTSTATUS)( Status ) ) >= 0 )
    ...
    {
        char msgBuffer[256];
        FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM, NULL, status, MAKELANGID( LANG_NEUTRAL, SUBLANG_DEFAULT ),
                       &msgBuffer[0], sizeof( msgBuffer ) - 1, nullptr );
...
#pragma pack( pop )
    ...
    FUNCTION_RETURN verify_signature( const std::string & stringToVerify, const std::string & signatureB64 )
    {
        ...
        if ( NT_SUCCESS( status = BCryptGetProperty( hash_alg, BCRYPT_OBJECT_LENGTH, (PBYTE)&cbHashObject,
                                                     sizeof( DWORD ), &cbData, 0 ) ) &&
             NT_SUCCESS( status = BCryptGetProperty( hash_alg, BCRYPT_HASH_LENGTH, (PBYTE)&cbHashDataLenght,
                                                     sizeof( DWORD ), &cbData, 0 ) ) ) {
            ...
            if ( NULL != pbHashObject && nullptr != pbHashData ) {
                if ( NT_SUCCESS( status =
                                     BCryptCreateHash( hash_alg, &hHash, pbHashObject, cbHashObject, NULL, 0, 0 ) ) ) {
