...
namespace license { namespace test {
    ...
    BOOST_AUTO_TEST_CASE( volid_lic_file )
    {
        ...
        if ( ( result_diskinfos == FUNC_RET_BUFFER_TOO_SMALL || result_diskinfos == FUNC_RET_OK ) &&
             diskInfos.size() > 0 ) {
    ...
    BOOST_AUTO_TEST_CASE( strategy_mac_address )
    {
        ...
        if ( ( result_adapterInfos == FUNC_RET_BUFFER_TOO_SMALL || result_adapterInfos == FUNC_RET_OK ) &&
             adapters.size() > 0 ) {
            generate_and_verify_license( LCC_API_HW_IDENTIFICATION_STRATEGY::STRATEGY_ETHERNET,
                                         "strategy_mac_address" );
        }
        else {
    ...
    BOOST_AUTO_TEST_CASE( strategy_ip_address )
    {
        ...
        if ( ( result_adapterInfos == FUNC_RET_BUFFER_TOO_SMALL || result_adapterInfos == FUNC_RET_OK ) &&
             adapters.size() > 0 ) {
            generate_and_verify_license( LCC_API_HW_IDENTIFICATION_STRATEGY::STRATEGY_IP_ADDRESS,
                                         "strategy_ip_address" );
        }
        else {
